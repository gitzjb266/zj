...
/**
 * oss 获取token 有效时间为24小时
 * 阿里错误码地址：https://help.aliyun.com/document_detail/174864.htm?spm=a2c4g.11186623.2.12.6de54e88AFTvVA
 * code=40020503 到控制台->人员管理->用户->选择具体的账号-添加权限->搜索《管理智能语音交互(NLS)的权限》->添加
 */
@Slf4j
public class TokenUtils {
    ...
    public static String canonicalizedQuery( Map<String, String> queryParamsMap) {
        ...
        try {
            ...
            log.info("规范化后的请求参数串：" + queryString);
        } catch (UnsupportedEncodingException e) {
            log.info("UTF-8 encoding is not supported.");
            e.printStackTrace();
        }
        return queryString;
    }
    ...
    public static String createStringToSign(String method, String urlPath, String queryString) {
        ...
        try {
            ...
            log.info("构造的签名字符串：" + stringToSign);
        } catch (UnsupportedEncodingException e) {
            log.info("UTF-8 encoding is not supported.");
            e.printStackTrace();
        }
    ...
    public static String sign(String stringToSign, String accessKeySecret) {
        try {
            ...
            log.info("计算的得到的签名：" + signBase64);
            String signUrlEncode = percentEncode(signBase64);
            log.info("UrlEncode编码后的签名：" + signUrlEncode);
    ...
    public static String getOssToken(String accessKeyId,String accessKeySecret) {
       // String accessKeyId = "LTAI5tNQjX9TZJGxdC8J2Asj";//args[0];
       // String accessKeySecret = "TjWqNUh17c5NcsdJzwIscl9JCgc3tA";//args[1];
        log.info(getISO8601Time(null));
        // 所有请求参数
        ...
        log.info("带有签名的请求字符串：" + queryStringWithSign);
        /**
         * 5.发送HTTP GET请求，获取token。
         */
        processGETRequest(queryStringWithSign);
        if (token != null) {
            log.info("获取的Token：" + token + ", 有效期时间戳（秒）：" + expireTime);
            // 将10位数的时间戳转换为北京时间
